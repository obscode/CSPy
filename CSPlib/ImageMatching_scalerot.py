''' imageMatching.py

    Software to optimally match the PSF of one image (a template) to another
    (the image) and then subtract them.

    AUTHOR:  Dan Kelson
    
    10/10/2005:  Added a bit of code to let the user specify masks in the
                 header, allowing them to throw out bad stars, ghosts, etc.
    02/21/2006:  We can now input a noise map (using -sig) so that the same
                 convolution can be done to an image's noise map.
    03/02/2010:  Added WCS code to allow for easier use.  Now, if both the image
                 and template have WCS in their headers, we can take out a lot of
                 the logic that had a tendency to fail with different instruments
                 and relative rotations.
   02/13/2020:  Ack, 10 years later!  Trying to port this to more common
                modules and python 3
'''
import time,os
#from FITS import *
from .fitsutils import qdump
#from Multipack import *
#from VTKHelperFunctions import *
from .ReadSex import *
#from masks import *
#from basics import *
from astropy.io import fits as FITS
import numpy.fft as fft
from numpy import int8
from numpy import savetxt,loadtxt,newaxis,sqrt,power,arange,ravel
from .GaussianBG import GaussianBG
#import convolve
debug = 1
try:
   import TwoPicker as Picker
   import FITSviewer
   from matplotlib import pyplot as plt
except:
   Picker = None
   FITSviewer = None
   plt = None
#from pygplot import columns
try:
   from astropy import wcs
except:
   wcs = None

try:
   import fit_psf
except:
   fit_psf = None

NA = newaxis
def QuickShift(i,x,y):
    '''Shift an image, i, by amount x and y.  Note 'x' is in the sense of the
    2nd index (image coordinates)'''
    s = VTKImageShift(i,x,y,interp=0,numret=1)
    return s

def ndf(nord):
   '''Returns the number of degrees of freedom for nord-order coordinate 
   transformation.'''
   if nord == -1:
      Ncoeff = 2
   else:
      Ncoeff = len(ravel(abasis(nord, array([0]), array([0]))))
   return Ncoeff

def headerVal(val, hdr, convert=float):
   '''If val is a header keyword, return it from the header.'''
   if type(val) is type(""):
      retval = hdr.getval(val)
      if retval == "N/A":
         raise ValueError(\
          "You gave me a %s header keyword, but I can't find it in the header" % (val))
   else:
      retval = val
   if convert is not None:
      return convert(retval)
   else:
      return retval

class Point:
   ''' A class that corresponds to a point on the image.  It can either be in
   pixel or world coordinates, and there are member functions to transform them
   when a WCS is present.'''

   def __init__(self, x, y, image=None):
      self.units = None
      if x is not None:
         if type(x) is type(""):
            if x[-1] == 'd':
               self.x = float(x[:-1])
               self.units = 'd'
            else:
               self.x = float(x)
               self.units = 'p'
         else:
            self.x = float(x)
            self.units = 'p'
      else:
         self.x = None

      if y is not None:
         if type(y) is type(""):
            if y[-1] == 'd':
               if self.units == "p":
                  raise ValueError("Can't mix units for X and Y")
               self.y = float(y[:-1])
               self.units = 'd'
            else:
               if self.units == 'd':
                  raise ValueError("Can't mix units for X and Y")
               self.y = float(y)
               self.units = 'p'
         else:
            if self.units == 'd':
               raise ValueError("Can't mix units for X and Y")
            self.y = float(y)
            self.units = 'p'
      if self.x is None and self.y is None:
         raise ValueError("Error, one of x or y must be specified")
      self.image = image

   def topixel(self):
      if self.units == 'p':
         return (self.x, self.y)
      else:
         if self.image is None or self.image.wcs is None:
            raise AttributeError("Must have A WCS before I can transform to pixels")
         if self.x is None:
            x = self.image.naxis1/2
         else:
            x = self.x
         if self.y is None:
            y = self.image.naxis2/2
         else:
            y = self.y
         i,j = self.image.wcs.wcs_world2pix(x, y, 0)
         if self.x is None:  i = None
         if self.y is None:  j = None
         return(i,j)

   def toworld(self):
      if self.units == 'd':
         return (self.x, self.y)
      else:
         if self.image is None or self.image.wcs is None:
            raise AttributeError("Must have A WCS before I can transform to Ra/DEC")
         if self.x is None:
            x = self.image.wcs.wcs.crval[0]
         else:
            x = self.x
         if self.y is None:
            y = self.image.wcs.wcs.crval[1]
         else:
            y = self.y
         x,y = self.image.wcs.wcs_pix2world(x, y, 0)
         if self.x is None:  x = None
         if self.y is None:  y = None

         return(x,y)

class Mask:
   '''An object that defines a mask for an image.'''

   def __init__(self, image):
      '''[image] is an Observation class.'''
      self.image = image
      self.llcs = []    # lower-left corners... point classes
      self.urcs = []    # upper-right corners... point classes
      self.sides = []   # 0 --> mask out interior  1 --> mask out exterior

   def __repr__(self):

      str = "Masks:\n"
      for i in range(len(self.llcs)):
         i0,j0 = self.llcs[i].topixel()
         i1,j1 = self.urcs[i].topixel()
         if self.sides[i] == 0:
            str += "\t[%.1f:%.1f,%.1f:%.1f] = False\n" % (i0,i1,j0,j1)
         else:
            str += "\t[%.1f:%.1f,%.1f:%.1f] = True\n" % (i0,i1,j0,j1)
      return str

   def add_mask(self, x0, y0, x1, y1, sense='inside'):
      '''Add a box mask.  If [sense] is 'inside' mask the interior pixels, 
      otherwise, mask the outside picels.'''
      if x1 < x0:  x0,x1 = x1,x0
      if y1 < y0:  y0,y1 = y1,y0
      try:
         p0 = Point(x0,y0,self.image)
         p1 = Point(x1,y1,self.image)
      except:
         raise ValueError("Problem with mask %s" % (str(x0,y0,x1,y1)))
      self.llcs.append(p0)
      self.urcs.append(p1)
      if sense.lower() == 'inside':
         self.sides.append(0)
      else:
         self.sides.append(1)

   def get_mask(self):
      '''Based on the current masks, return the end result.'''
      ox = getattr(self.image, 'ox', None)
      oy = getattr(self.image, 'oy', None)
      if ox is None or oy is None:
         oy,ox = indices((self.image.naxis2,self.image.naxis1),Float32)
      mask = ones((self.image.naxis2,self.image.naxis1), Float32)
      for i in range(len(self.llcs)):
         i0,j0 = self.llcs[i].topixel()
         i1,j1 = self.urcs[i].topixel()
         print(i0,j0,i1,j1)
         if i0 > i1:  i0,i1 = i1,i0
         if j0 > j1:  j0,j1 = j1,j0
         if self.sides[i] == 1:
            if i0 and i1:
               mask = VTKMultiply(mask, between(ox,i0,i1).astype(Float32))
            if j0 and j1:
               mask = VTKMultiply(mask, between(oy,j0,j1).astype(Float32))
         else:
            outsidex = VTKOr(less(ox,i0).astype(Float32),
                             greater(ox,i1).astype(Float32))
            outsidey = VTKOr(less(oy,j0).astype(Float32),
                             greater(oy,j1).astype(Float32))
            mask = VTKMultiply(mask, VTKOr(outsidex,outsidey))
      return mask

   def flag_pixels(self, x, y):
      ''' Given x,y points, determined if they are masked (False) or not masked
      (True).'''
      gids = ones(x.shape, bool)
      for i in range(len(self.llcs)):
         i0,j0 = self.llcs[i].topixel()
         i1,j1 = self.urcs[i].topixel()
         if i0 > i1:  i0,i1 = i1,i0
         if j0 > j1:  j0,j1 = j1,j0
         if self.sides[i] == 1:
            if i0 and i1:
               gids = gids*between(x, i0, i1)
            if j0 and j1:
               gids = gids*between(y, j0, j1)
         else:
            gids1 = less(x, i0) + greater(x, i1)
            gids2 = less(y, j0) + greater(y, j1)
            gids = gids*(gids1+gids2)
      return gids

   def dump_mask(self, file):
      qdump(file, self.get_mask())


class Observation:
    def __init__(self,image,wt=None,scale='scale',pakey="rotangle",
          saturate=30000, skylev=None, sigsuf=None, wtype="MAP_RMS", 
          mask_file=None, reject=0, snx=None, sny=None, snmaskr=10.0, 
          extra_sufs=[], hdu=0):

       self.image = image   # The image name
       self.hdu = hdu

       # Keep a logfile
       self.log_stream = open(self.image.replace('.fits','')+'.log', 'w')

       base = os.path.basename(image)
       self.catalog = base.replace(".fits",".cat")  # Output catalog of objects
       if sigsuf: 
          self.sigimage = self.image.replace(".fits",sigsuf+".fits")
       else: 
          self.sigimage = None
       if extra_sufs:
          self.extra_maps = [self.image.replace(".fits", suf+".fits") \
                for suf in extra_sufs]
       else:
          self.extra_maps = None
       self.wtype=wtype
       self.template = base.replace(".fits","temp.fits")   # Name of template
       self.rectemp = base.replace(".fits","master.fits")  # Rectified template
       self.recsigma = base.replace(".fits","master_sigma.fits") # rec'd sigma
       self.SN = base.replace(".fits","SN.fits")           # Name of SN frame
       self.imbases = base.replace(".fits","bases.fits")   # Output kernelbases
       self.bimage = base.replace(".fits","bg.fits")       # OUtput background
       self.nimage = base.replace(".fits","noise.fits")    # Output Noise
       self.psfgrid = base.replace(".fits","grid.fits")    # Output kernel
       self.masked = base.replace(".fits","mask.fits")     # Image Mask
       self.difference = base.replace(".fits","diff.fits") # difference image
                                                            # Segmentation map:
       self.SNpostage = base.replace(".fits","SNpost.fits") # 400X400 SN image
       self.diffpostage = base.replace(".fits","diffpost.fits") # 400X400 difference image
       self.db = base.replace(".fits", "db.txt")    # Object database for matching
       self.rdv = base.replace(".fits","rdv.fits") # relative eigenvalues.
       self.segmap=base.replace(".fits","seg.fits")
       self.wt = wt                      # Weights
       self.bg = 0.0                     # Background
       self.saturate = saturate          # saturation value in image

       self.reject = reject              # completely reject saturated objects?
       f = FITS.open(self.image)
       self.epoch = f[hdu].header.get("epoch", "N/A")
       self.exptime = f[hdu].header.get("exptime", "N/A")
       if self.epoch == "N/A": self.epoch = -99
       if self.exptime == "N/A": self.exptime = 1.0
       if type(scale) is type(""):
          self.scale = f[hdu].header.get(scale, "N/A")
       else:
          self.scale = scale
       if skylev is not None:
          self.skylev = f[hdu].header.get(skylev, "N/A")
          self.saturate = self.saturate - self.skylev
       else:
          self.skylev = 0

       self.starlist = None

       if snx is not None and sny is not None:
          try:
             self.snpos = Point(snx, sny, self)
          except:
             snx = f[hdu].header.get(snx, "N/A")
             sny = f[hdu].header.get(sny, "N/A")
             self.snpos = Point(snx,sny, self)
       else:
          self.snpos = None
       self.snmaskr = snmaskr


       self.crowd = False

       self.mask = Mask(self)
       # Added by CRB:  Allow the user to mask out data.
       if mask_file is not None:
          mf = open(mask_file,'r')
          for line in mf.readlines():
             x0,y0,x1,y1 = line.split()
             self.mask.add_mask(x0,y0,x1,y1, sense='inside')
             self.log('Adding mask: %s %s %s %s' % (x0,y0,x1,y1))
          mf.close()
       #self.ra = f["ra"]
       #self.dec = f["dec"]
       self.pa = f[hdu].header.get(pakey, "N/A")
       if self.pa == "N/A": self.pa = 0.0
       #if type(self.ra) == StringType: self.ra = 15.0*scanSexagesimal(self.ra)
       #if type(self.dec) == StringType: self.dec= scanSexagesimal(self.dec)
       f.close()
       self.data = None
       self.master = None
       # Take this out:  it's redundant
       #self.user_masks = 0

    def log(self, message):
       '''Quick function to print log information to the screen and also keep a copy
       in a specified log file.'''
       print(message)
       if self.log_stream is not None:
          #print >> self.log_stream, message
          self.log_stream.write(message+'\n')


    def __repr__(self):
       '''Give the user a nice representation of the object.'''
       return self.image


    def sex(self):
       '''This function runs sextractor on the image, finding images to be used
       for rectification and solving for the kernel.'''

       if self.sexdir[-1] != "/": self.sexdir += "/"
       self.sexcom = ["sex",self.image]
       self.sexcom += ["-c "+self.sexdir+"default.sex"]
       self.sexcom += ["-PARAMETERS_NAME "+self.sexdir+"default.param"]
       if self.sigimage: self.sexcom += ["-WEIGHT_IMAGE "+self.sigimage]
       if self.sigimage: self.sexcom += ["-WEIGHT_TYPE %s" % self.wtype]
       self.sexcom += ["-DETECT_MINAREA 5"]
       self.sexcom += ["-DETECT_THRESH "+str(self.thresh)]
       self.sexcom += ["-ANALYSIS_THRESH "+str(self.thresh)]
       self.sexcom += ["-PIXEL_SCALE "+str(self.scale)]
       self.sexcom += ["-SEEING_FWHM "+str(0.8/self.scale)]
       self.sexcom += ["-CATALOG_NAME",self.catalog]
       self.sexcom += ["-CHECKIMAGE_TYPE SEGMENTATION"]
       self.sexcom += ["-CHECKIMAGE_NAME "+self.segmap]
       self.sexcom += ["-FILTER_NAME "+self.sexdir+"gauss_3.0_5x5.conv"]
       self.sexcom += ["-STARNNW_NAME "+self.sexdir+"default.nnw"]
       self.sexcom += ["-SATUR_LEVEL "+str(self.saturate)]
       self.sexcom += ["-VERBOSE_TYPE QUIET"]
       self.sexcom = ' '.join(self.sexcom)
       #if self.verb > 1:
       #   verbex(self.sexcom)
       #else:
       os.system(self.sexcom)

    def readcat(self):
       '''Read the SExtractor output.'''
       if self.do_sex:  self.sex()
       self.sexdata = readsex(self.catalog)[0]
       print(self.segmap)
       f = FITS.open(self.segmap)
       print(len(f))
       self.seg = f[self.hdu].data
       f.close()

    def compute(self,nmax=40, min_sep=None):
       '''Based on the data output by SExtractor, build arrays of the distances
       and angles between all the objects in the catalog.  Used later to match
       up the objects to figure out the coordinate transformation from image
       to template.'''
       self.nmax = nmax
       self.min_sep = min_sep
       x = self.sexdata["X_IMAGE"]
       y = self.sexdata["Y_IMAGE"]
       if "MAG_BEST" in self.sexdata: m = self.sexdata["MAG_BEST"]
       elif "MAG_AUTO" in self.sexdata: m = self.sexdata["MAG_AUTO"]
       q = self.sexdata["FLAGS"]
       q = array([int(qq) for qq in q])
       cids = less_equal(q,4)
       if not self.crowd:
          uncrowd = equal(bitwise_and(q,1), 0)
          cids = cids*uncrowd
       x,y,m,q = compress(cids,[x,y,m,q],1)
       x = x - 1
       y = y - 1
       # If the FITS header had masks in the header, then we mask that out too
       #  This is opposite of xwin,ywin:  it's want we want to keep OUT.
       #if self.mask:
       ids = self.mask.flag_pixels(x,y)
          # for ma in self.im_masks:
          #    self.log("Applying mask [x1:x2,y1:y2]=[%d:%d,%d:%d]" % tuple(ma))
          #    condx = less(x, ma[0]) + greater(x, ma[1])
          #    condy = less(y, ma[2]) + greater(y, ma[3])
          #    ids = nonzero(condx + condy)
       #x,y,m,q = take([x,y,m,q],ids,1)
       x = x[ids];  y = y[ids];  m = m[ids];  q = q[ids]
       if self.min_sep is not None:
          dx = x[NewAxis,::] - x[::,NewAxis]
          dy = y[NewAxis,::] - y[::,NewAxis]
          ds = sqrt(power(dx,2) + power(dy,2))
          # number of neighbors within min_sep of each point
          num_nn = sum(less(ds, self.min_sep/self.scale), axis=1) - 1
          ids = nonzero(equal(num_nn, 0))
          x,y,m,q = take([x,y,m,q], ids, 1)

       a = self.pa*pi/180
       #r = self.ra*pi/180
       #d = self.dec*pi/180
       o = argsort(m)[:self.nmax]
       x,y,m = take([x,y,m],o,1)
       dx = x[NewAxis,::] - x[::,NewAxis]
       dy = y[NewAxis,::] - y[::,NewAxis]
       ds = sqrt(power(dx,2) + power(dy,2))
       da = arctan2(dy,dx)*180/pi
       self.x = x
       self.y = y
       self.m = m
       self.dx = dx*self.scale
       self.dy = dy*self.scale
       self.ds = ds*self.scale
       self.da = da
     
     
    def objmatch(self,nord=0,perr=2,aerr=1.0, angles=[0.0],interactive=0,
          use_db=0):
       '''Figure out which objects matches up with which other objects in the
       image and template (master) frames.'''
       self.log("Matching "+self.master.image+" to "+self.image)
       if "sexdata" not in dir(self.master):
          self.master.master = self.master
          self.master.readcat()
          self.master.compute(nmax=self.nmax, min_sep=self.min_sep)

       # Now we match objets to objects. There are several ways to do this...
       if use_db:
          if os.path.isfile(self.db) and os.path.isfile(self.master.db):
             x0,y0 = loadtxt(self.db, unpack=True)
             x1,y1 = loadtxt(self.master.db, unpack=True)
          else:
             self.log('Error:  to use -db, the database files must exist')
             sys.exit(1)
          Niter = 1
       elif interactive:
          if Picker is None:
             raise RuntimeError("Sorry, to work interactively, you need matplotlib.")
          picker = Picker.FITSpicker(self.image, self.master.image)
                #self.scale, self.master.scale)
          plt.show()
          x0,y0,x1,y1 = picker.get_points()
          Niter = 1
       elif self.starlist is not None:
          # User has provided a list of stars to use. We need WCS to get proper
          # coordinates
          if self.wcs is None or self.master.wcs is None:
             raise RuntimeError("In order to use a star list, you need WCS information"
                                 " in both frames")
          if fit_psf is None:
             raise RuntimeError("In order to use a star list, you need the fit_psf "
                                 "module.")
          x0,x1,y0,y1 = [],[],[],[]
          #print self.starlist
          for xi,yi in self.starlist:
             [[i0,j0]] = self.wcs.wcs_world2pix([[xi,yi]],1)
             [[i1,j1]] = self.master.wcs.wcs_world2pix([[xi,yi]],1)
             if not (10 < i0 < self.data.shape[1]-10 and \
                     10 < j0 < self.data.shape[0]-10 and \
                     10 < i1 < self.master.data.shape[1]-10 and \
                     10 < j1 < self.master.data.shape[0]-10):
                continue
             pars = fit_psf.fit_psf(self.data, i0, j0, 1.0/self.scale, 2.0)
             pars = fit_psf.fit_psf(self.master.data, i1, j1, 1.0/self.scale, 2.0)
             x0.append(i0); y0.append(j0)
             x1.append(i1); y1.append(j1)
          x0,x1,y0,y1 = map(array, [x0,x1,y0,y1])

          Niter = 1
       else:
          if self.wcs is not None and self.master.wcs is not None:
             # Woo-hoo!  We've got a WCS.  This will make matching much
             #  easier.
             uv0 = self.wcs.wcs_pix2world(transpose([self.x,self.y]),1)
             uv1 = self.master.wcs.wcs_pix2world(
                   transpose([self.master.x,self.master.y]),1)
             savetxt('uv0', uv0)
             savetxt('uv1', uv1)
             # dists[i,j] gives the absolute distance from object i in the image to 
             #object j in master
             dists = sqrt(power(uv0[:,0,NA] - uv1[NA,:,0],2) + power(uv0[:,1,NA] - uv1[NA,:,1],2))
             dists = dists*3600/self.scale
             gids = less(dists, perr)
             if sum(greater(sum(gids, axis=1),0)) < 3:
                self.log("Found fewer than 3 matches, try increasing match tolerance (-s)")
                return -1
             #uids0 = nonzero(equal(sum(gids, axis=1),1))
             uids0 = greater(sum(gids, axis=1),0)
             #uids1 = array([nonzero(gids[i,:])[0] for i in uids0])
             uids1 = argmin(dists, axis=1)[uids0]
             x0 = self.x[uids0];  y0 = self.y[uids0]
             x1 = self.master.x[uids1];  y1 = self.master.y[uids1]
          else:
             # Do it the hard way...
             best_N = -1
             best_a = 0
             for aoffset in angles:
                da = self.da[::,::,NA,NA] - self.master.da[NA,NA,::,::] + aoffset
                da = where(less(da, -180.0), da+180.0, da)
                da = where(greater(da, 180.0), da-180.0, da)
                ds = self.ds[::,::,NA,NA] - self.master.ds[NA,NA,::,::]
                use = less(abs(ds),perr)*less(abs(da),aerr)
                suse = add.reduce(add.reduce(use,3),1)
                if max(ravel(suse)) < 4:  continue
                guse = greater(suse,max(ravel(suse))/2)
                self.log("angle %.2f gives %d matches" % (aoffset, sum(ravel(guse))))
                if sum(ravel(guse)) > best_N:
                   best_a = aoffset
                   best_N = sum(ravel(guse))
             da = self.da[::,::,NA,NA] - self.master.da[NA,NA,::,::] + best_a
             ds = self.ds[::,::,NA,NA] - self.master.ds[NA,NA,::,::]
             use = less(abs(ds),perr)*less(abs(da),aerr)
             suse = add.reduce(add.reduce(use,3),1)
             if self.verb > 1:
                for suse1 in suse: self.log(suse1)
             guse = greater(suse,max(ravel(suse))/2)
             if self.verb:
                self.log('found %d matches' % sum(ravel(guse)))
                if best_a != 0:
                   self.log('best angular offset = %.2f' % best_a)
             i = [j for j in range(self.x.shape[0]) if sum(guse[j])]
             m = [argmax(guse[j]) for j in range(self.x.shape[0]) if sum(guse[j])]
             x0,y0 = take([self.x,self.y],i,1)
             x1,y1 = take([self.master.x,self.master.y],m,1)
             rscale = self.master.scale/self.scale
             best_a = best_a*pi/180.0
             xt1 = rscale*(cos(best_a)*x1 + sin(best_a)*y1)
             yt1 = rscale*(-sin(best_a)*x1 + cos(best_a)*y1)
             xshift,xscat = bwt(x0-xt1)
             xscat = max([1.0,xscat])
             yshift,yscat = bwt(y0-yt1)
             yscat = max([1.0,yscat])
             keep = less(abs(x0-xt1-xshift),3*xscat)*less(abs(y0-yt1-yshift),3*yscat)
             x0,y0,x1,y1 = compress( keep, [x0,y0,x1,y1], 1)
          Niter = 3


       Nmatches = len(x0)
       if Nmatches < 3:
          self.log("Found fewer than 3 matches.  Aborting.")
          return -1
       Ncoeff = ndf(nord)
       if 2*Nmatches < Ncoeff:
          nord0 = nord
          self.log("You have fewer than %d matches, trying a lower " % (Ncoeff/2) +\
                "order transformation")
          nord -= 1
          while 2*Nmatches < ndf(nord):
             nord -= 1
          self.log("Using order %d transformation" % (nord))


       wt = ones(x0.shape,Float32)

       if self.verb > 1 and FITSviewer is not None:
          fig = FITSviewer.FITSviewer(self.master.image)
          fig.label_objs(x1,y1, radius=5)
          plt.draw()

       # now we iterate on the solution and throw out bad points
       for iter in range(Niter):
          if iter:
             allx0 = array(self.sexdata["X_IMAGE"])
             ally0 = array(self.sexdata["Y_IMAGE"])
             if "MAG_BEST" in self.sexdata: 
                allm0 = array(self.sexdata["MAG_BEST"])
             elif "MAG_AUTO" in self.sexdata: 
                allm0 = array(self.sexdata["MAG_AUTO"])
             allq0 = array(self.sexdata["FLAGS"])
             allx1 = array(self.master.sexdata["X_IMAGE"])
             ally1 = array(self.master.sexdata["Y_IMAGE"])
             if "MAG_BEST" in self.sexdata:
                allm1 = array(self.master.sexdata["MAG_BEST"])
             elif "MAG_AUTO" in self.sexdata: 
                allm1 = array(self.master.sexdata["MAG_AUTO"])
             allq1 = array(self.master.sexdata["FLAGS"])
             if nord == -1:
                ix = allx0 + xshift
                iy = ally0 + yshift
             else:
                basis = abasis(nord,allx0,ally0,rot=[0,1][nord==0])
                ixy = add.reduce(sol[NewAxis,::]*basis,1)
                ix,iy = ixy[:len(ravel(allx0))], ixy[len(ravel(allx0)):]
             if self.verb > 1 and FITSviewer is not None:
                fig.label_objs(allx1, ally1, radius=8, color=['black','blue','green'][iter])
                fig.label_objs(ix, iy, radius=8, color=['black','blue','green'][iter])
                plt.draw()
             delx = ix[::,NA] - allx1[NA,::]
             dely = iy[::,NA] - ally1[NA,::]
             dels = sqrt(power(delx,2) + power(dely,2))
             delq = less_equal(allq0,4)[::,NA]*less_equal(allq1,4)[NA,::]
             dels = where(delq,dels,9999)
             ui0 = [j for j in range(delx.shape[0]) if min(dels[j]) < perr]
             ui1 = [argmin(dels[j]) for j in range(delx.shape[0]) \
                   if min(dels[j]) < perr]
             if len(ui0) == 0:
                self.log("Error:  Residuals of coordinate tranformation are all greater")
                self.log("than one pixel.  Try with smaller order.")
                if self.verb > 1:
                   fig.save('match.eps')
                return -1
             x0,y0,m0 = take([allx0,ally0,allm0],ui0,1)
             x1,y1,m1 = take([allx1,ally1,allm1],ui1,1)
             f0 = power(10,-0.4*(m0-min(m0)))
             f1 = power(10,-0.4*(m1-min(m1)))
             wt = sqrt(f0+f1) * 0.0 + 1
          if nord == -1:
             xshift = sum(wt*(x1-x0))/sum(wt)
             yshift = sum(wt*(y1-y0))/sum(wt)
             sol = array([xshift,yshift,1.0,0.0])
             ix = x0 + xshift
             iy = y0 + yshift
          else:
             wt = concatenate([wt,wt])
             basis = abasis(nord,x0,y0,rot=[0,1][nord==0])
             sol = svdfit(basis*wt[::,NA],concatenate([x1,y1])*wt)
             ixy = add.reduce(sol[NewAxis,::]*basis,1)
             ix,iy = ixy[:len(ravel(x0))], ixy[len(ravel(x0)):]

          if self.verb > 1 and FITSviewer is not None and not iter:
             fig.label_objs(ix, iy, radius=8, color='black')
             plt.draw()

          self.log( "Pass %d, with %d objects." % (iter+1,len(x0)))
          if nord==-1:
             self.log("  xshift: %.3f   yshift:  %.3f" % tuple(sol[0:2]))
          elif nord == 0:
             theta = arctan2(sol[3],sol[2])
             scale = sol[2]/cos(theta)
             self.log(" xshift: %.3f yshift: %.3f scale: %.3f rot:  %.3f" %\
                   (sol[0],sol[1],scale,theta*180.0/pi))
          else:
             self.log( sol)
          delx = ix-x1
          dely = iy-y1
          dels = sqrt(power(delx,2) + power(dely,2))
          scx = bwt(delx)[1]
          scy = bwt(dely)[1]
          self.log( "Biweight estimate for scatter in coordinate trans. (x,y) = (%.5f,%.5f)" % (scx,scy))
          tol = 3.0*bwt(dels)[1]

       if self.verb > 1 and FITSviewer is not None:
          fig.save('match.eps')
       if not use_db:
          f = open(self.db, 'w')
          for i in range(len(x0)):
             #print >>f, x0[i],y0[i]
             f.write("{} {}\n".format(x0[i],y0[i]))
          f.close()
          f = open(self.master.db, 'w')
          for i in range(len(x0)):
             #print >>f, x1[i],y1[i]
             f.write("{} {}\n".format(x1[i],y1[i]))
          f.close()

       self.sol = sol
       self.nord = nord
       return 0

    def imread(self, bs=False, usewcs=False):
        '''Read in the FITS data and assign to the member variables.  Optionall background
        subtract if bs=True'''
        if self.data == None:
           self.log( "Now reading frames for %s" % (self))
           f = FITS.open(self.image)
           self.data = f[self.hdu].data
           # see if there is WCS info in the header
           if usewcs and wcs is not None:
              self.wcs = wcs.WCS(f[self.hdu].header)
              #if self.wcs.types[0] is None or self.wcs.types[1] is None:
              #   # Bad WCS
              #   self.wcs = None
              self.log('Loading WCS info for %s' % self.image)
              #if self.snra is not None and self.sndec is not None:
              #   self.snx,self.sny = self.wcs.topixel((self.snra,self.sndec))
           else:
              self.wcs = None
           if bs:
              self.data = self.data - GaussianBG(ravel(self.data).astype(float32),101)
           self.naxis1,self.naxis2 = f[self.hdu].header['NAXIS1'],f[self.hdu].header['NAXIS2']
           f.close()
           if self.master: 
              self.master.imread(bs=bs, usewcs=usewcs)

           if self.sigimage is not None:
              self.log( "Reading in sigma map %s" % self.sigimage)
              f = FITS.open(self.sigimage)
              self.sigma = f[self.hdu].data
              f.close()
           else:
              self.sigma = None

           if self.extra_maps is not None:
              self.extras = []
              for xmap in self.extra_maps:
                 self.log("Reading in extra map %s" % xmap)
                 f = FITS.open(xmap)
                 self.extras.append(f[self.hdu].data)
                 f.close()
           else:
              self.extras = None


    def mktemplate(self,registered,usewcs=False,sol=None,mimage=None):
        '''Transform the template (master) to the coordinates of the image.'''
        self.oy,self.ox = indices((self.naxis2,self.naxis1),Float32)

        # first, reject master segmented objects before we transform
        if self.reject:
           # reject full objects if requested
           sat_pixels_m = greater(self.master.data, self.master.saturate)
           sat_ids_m = nonzero(ravel(sat_pixels_m))
           sat_objects_m = ravel(self.master.seg)[sat_ids_m]
           # Now a bit of trickery to find non-repeating set of object numbers:
           objects_m = list(set(sat_objects_m))
           for obj in objects_m:
              self.master.seg[equal(self.master.seg,obj)] = 0

        if registered:
           self.timage = self.master.data
           if self.master.sigimage is not None:
              self.tsigma = self.master.sigma
           else:
              self.tsigma = None
           if self.master.extras is not None:
              self.textras = [xmap for xmap in self.master.extras]
           else:
              self.textras = None
           self.mseg = greater(self.master.seg, 0.0).astype(Float32)
        else:
           if not mimage: mimage = self.master.data
           if usewcs and self.wcs is not None and self.master.wcs is not None:
              # Here we don't bother with our own coordinate solution,
              #  we use the WCS in each image to do the coordinate
              #  transformation
              uv0 = self.wcs.toworld(transpose([self.ox,self.oy]))
              ixy = self.master.wcs.topixel(uv0).T
              ix = ixy[0];  iy = ixy[1]
              self.tx = ix - self.ox
              self.ty = iy - self.oy
           else:
              if not sol: sol = self.sol
              self.log( "Constructing transformation...")
              ix,iy = mbasis(sol,self.ox,self.oy,rot=[0,1][self.nord<=0])
              self.tx = ix - self.ox
              self.ty = iy - self.oy
           self.log( "Transforming...")
           self.timage = VTKImageTransform(mimage,self.tx,self.ty,numret=1,
                                           cubic=0,interp=1,constant=0)
           if self.master.sigimage is not None:
              self.log( "Transforming sigma map...")
              bpm = less(self.master.sigma,0)*1.0
              self.tsigma = VTKImageTransform(self.master.sigma, self.tx, 
                    self.ty, numret=1, cubic=0, interp=1, constant=-1)
              tbpm = VTKImageTransform(bpm, self.tx, 
                    self.ty, numret=1, cubic=0, interp=1, constant=-1)
              self.tsigma = where(tbpm > 0.1, -1, self.tsigma)
           else:
              self.tsigma = None

           if self.master.extras is not None:
              self.log( "Transforming extra maps...")
              self.textras = []
              for xmap in self.master.extras:
                 self.textras.append(VTKImageTransform(xmap, self.tx,
                      self.ty, numret=1, cubic=0, interp=1, constant=-1))
           else:
              self.textras = None

           mseg = greater(self.master.seg, 0.0).astype(Float32)
           if self.master.mask:
              mseg = VTKMultiply(mseg, self.master.mask.get_mask())
           self.mseg = VTKImageTransform(mseg,self.tx,self.ty,numret=1,
                                         cubic=0,interp=1,constant=-1)
           qump('mseg.fits', self.mseg, self.image)

    def estimate_bg(self, Niter=5):
        '''Estimate the background level.'''
        for uk in range(Niter):
           if uk:
              ukeep = 1.0*between(self.data, bg-4*r, bg+2*r)
              ukeep = 1.0*equal(VTKConvolve(ukeep, k=5, numret=1),1.0)
              udata = compress(ukeep.ravel(), self.data.ravel())
           else:
              udata = self.data.ravel()
           
           bg = GaussianBG(udata,99)
           d = VTKSubtract(udata, bg)
           r = 1.49*median(abs(d))
           if self.verbose > 0:
              print("Using %d: BG=%.3f with sigma=%.3f" % (len(udata),bg,r))
        return bg,r

    def mkweight(self):
        '''This function works out the weight of the image, based on the noise
        in the background and the segmentation found by sextractor.  This is
        where you want to fiddle with which parts of the image make the most
        contribution to the final kernel solution.'''

        self.log( "Working on mask and weights...")
        # These are maps of where sextractor found objects, seg for this image
        #  and mseg for the master (template)

        if self.reject:
           # First, we figure out which objects have saturated pixels in them:
           sat_pixels = greater(self.data, self.saturate)
           sat_ids = nonzero(ravel(sat_pixels))
           sat_objects = ravel(self.seg)[sat_ids]
           # Now a bit of trickery to find non-repeating set of object numbers:
           objects = list(set(sat_objects))

           # Now cut out the objects with saturated pixels.
           for obj in objects:
              self.seg[self.seg == obj] = 0

        seg = greater(self.seg,0.0).astype(Float32)
        mseg = greater(self.mseg,0.0).astype(Float32)

        gwid = max([2,1*self.pwid])
        self.bg,r = self.estimate_bg()
        self.tbg,r2 = self.master.estimate_bg()
        #qdump(self.bimage,self.data-bg,self.image)
        print("Cutting below 5-sigma: %.3f" % (self.bg-5*r))
        self.log( "Image:" )
        self.log( "Using BG=%.3f with sigma=%.3f" % (self.bg,r))
        self.log( "Cutting below 5-sigma: %.3f" % (self.bg-5*r))
        self.log( "Template:" )
        self.log( "Using BG=%.3f with sigma=%.3f" % (self.tbg,r2))
        self.log( "Cutting below 5-sigma: %.3f" % (self.tbg-5*r2))
        # Lower-cut. Reject data less than lcut

        zids = equal(self.data, 0.0).astype(float32)
        zids = VTKGauss(zids, gwid, numret=1)
        # note by using less(zids, 0.02), we're effectively doing a fuzzy 'not'
        wt = VTKMultiply(seg, VTKMultiply(mseg, less(zids, 0.02)))

        lowids = less(self.data, self.bg-5*r).astype(float32)
        lowids = VTKGAuss(lowids, gwid, numret=1)
        swt = less(lowids, 0.02).astype(float32)
        qdump('swt.fits', swt, self.image)
        lowids = less(self.timage, self.tbg-5*r2).astype(float32)
        lowids = VTKGauss(lowids, gwid, numret=1)
        twt = less(lowids, 0.02).astype(float32)
        qdump('twt.fits', twt, self.image)
        wt = VTKMultiply(greater(VTKGauss(wt,gwid,numret=1),
                                 0.02).astype(float32),swt*twt)
        insidex = between(self.ox,2*gwid,self.naxis1-2*gwid).astype(float32)
        insidey = between(self.oy,2*gwid,self.naxis2-2*gwid).astype(float32)
        wt = VTKMultiply(wt,VTKAnd(insidex, insidey))

        # If the FITS header had masks in the header, then we mask that out too
        #  This is opposite of xwin,ywin:  it's want we want to keep OUT.
        # Note that xwin and ywin are now included in get_mask
        wt = VTKMultiply(wt, self.mask.get_mask())

        if self.snpos is not None:
           self.log("Applying super nova mask at x=%f, y=%f" % (self.snpos.topixel()))
           snx,sny = self.snpos.topixel()
           dists = power(self.ox-snx, 2) + power(self.oy-sny, 2)
           cond = greater(dists, (self.snmaskr/self.scale)**2)
           wt = VTKMultiply(wt, cond.astype(Float32))

        swt = VTKGreaterEqual(self.data,self.saturate)
        twt = VTKGreaterEqual(self.timage,self.master.saturate)
        swt = VTKDilate(VTKOr(swt,twt),5,5,numret=1)
        swt = VTKGauss(swt,gwid,numret=1)
        wt = VTKMultiply(wt, VTKLessEqual(swt,0.02))
        if self.sigimage:
           self.noise = self.sigma
        else:
           n2 = VTKSubtract(self.data,self.bg)
           self.noise = VTKSqrt(VTKAdd(n2,pow(r,2)))
        qdump(self.nimage,self.noise,self.image)
        self.invnoise = VTKInvert(self.noise)
        wt = VTKMultiply(wt, self.invnoise)
        if self.pwid > -1: 
           wt = VTKIslandRemoval(wt,1.0,0.0,max([3,self.pwid])**2,numret=1)
        self.wt = wt.astype(float32)


    def mkmatch(self,preserve,quick_convolve=0):
        '''Here's where the kernel is solved.  Need to work on comments here.
        Right now, it's pretty black-box.'''
        wtflat = ravel(self.wt)

        # Try to get counts on same scale
        self.timage = self.timage * (self.exptime/self.master.exptime)
        self.log( "Using %d pixels." % (sum(1.0*greater(wtflat,0))))
        if self.pwid == -1:
           flux = sum(wtflat*ravel(self.data))/sum(wtflat*ravel(self.timage))
           self.log( "Flux ratio = %.4f" % (flux))
           self.match = flux*self.timage
           if self.sigimage is not None and self.master.sigimage is not None:
              self.csigma = sqrt(power(self.sigma,2) + power(self.tsigma, 2))
        else:
           # Full width of the kernel, make it odd
           pful = int(2*self.pwid+1)
           owid = self.pwid + 1.0
           # These are indices of a box containing the kernel
           pbox = arange(-self.pwid,self.pwid+0.5,1.0).astype(Int32)
           # These are the indices in the pbox that correspond to being
           # inside a circular aperture.
           self.ii = array([i for i in pbox for j in pbox \
                            if (i*i+j*j)<=owid**2 or 0])
           self.jj = array([j for i in pbox for j in pbox \
                            if (i*i+j*j)<=owid**2 or 0])
           ll = len(self.ii)
           self.log( "Constructing bases.")
           # The construction of bases depends on the image data or the template
           #  (depending on direction), whether or not there is a sky offset
           #  and whether or not there is spatial variation.
           it0 = time.time()
           basis_sigma = None
           if self.rev:
              f0 = compress(wtflat,wtflat*ravel(self.timage))
              n0 = compress(wtflat,ravel(self.invnoise))
              basisimage = self.data
              if self.sigimage is not None:
                 # make a variance map 
                 basis_sigma = power(self.sigma, 2)
           else:
              f0 = compress(wtflat,wtflat*ravel(self.data))
              n0 = compress(wtflat,ravel(self.invnoise))
              basisimage = self.timage
              if self.master.sigimage is not None:
                 basis_sigma = power(self.tsigma, 2)
           step = ll/79 + 1  # for performance meter
           cwt = compress(wtflat,wtflat)
           # i,j indeces of masked image
           coy,cox = compress(wtflat, [ravel(self.oy), ravel(self.ox)]).astype(Int32)
           flatimage = ravel(basisimage)
           basis = []
           for k in range(ll):
              basis.append(cwt*take(flatimage,(coy+self.jj[k])*self.naxis1 + \
                    (cox+self.ii[k])))
              if k % step == 0:
                 sys.stdout.write(".")
                 sys.stdout.flush()

           if self.skyoff: basis = concatenate([basis,[ones(f0.shape,Float64)]])
           basis = asarray(basis)
           if self.spatial:
               self.oxl = (self.ox-self.naxis1/2.0)/(self.naxis1/2.0)
               woxl = compress(wtflat,ravel(self.oxl))
               self.oyl = (self.oy-self.naxis2/2.0)/(self.naxis2/2.0)
               woyl = compress(wtflat,ravel(self.oyl))
               xbasis = [b*woxl for b in basis[:-1]]
               ybasis = [b*woyl for b in basis[:-1]]
               basis = concatenate([basis,xbasis,ybasis])
           it1 = time.time()
           self.log( "Bases constructed in %.4fs." % (it1-it0))
           self.log( "Decomposition.")
           it0 = time.time()
           self.log( "Size of basis matrix:")
           self.log(basis.shape)
           du,dv,dw = singular_value_decomposition(transpose(basis))
           it1 = time.time()
           self.log( "Decomposition in %.4fs." % (it1-it0))
           bases = []
           for sol1a in dw:
               psf2 = zeros((pful,pful),Float64)
               for k in range(ll):
                   py = self.jj[k]+self.pwid
                   px = self.ii[k]+self.pwid
                   pr = sqrt(self.jj[k]**2 + self.ii[k]**2)
                   psf2[py,px] = sol1a[k]
               bases.append(psf2)
           bases = asarray(bases)
           npsx = int(sqrt(bases.shape[0]))+1
           npsy = bases.shape[0]/npsx + 1
           imbases = zeros( (npsy*(bases.shape[1]+5), npsx*(bases.shape[2]+5)),
                           Float64)
           imb1 = 0
           for j in range(npsy):
             for i in range(npsx):
                y0 = j*(bases.shape[1]+5)
                y1 = y0 + bases.shape[1]
                x0 = i*(bases.shape[2]+5)
                x1 = x0 + bases.shape[2]
                imbases[y0:y1,x0:x1] = bases[imb1,::-1,::]
                imb1 += 1
                if imb1 == len(bases): break
           imbases = imbases[::-1]
           qdump(self.imbases,imbases,self.image)
           if self.verb > 1: self.log( dv)
           sol1 = transpose(dw)
           if self.mcut and self.mcut < len(dv):
               self.log( "Ratio of first eigenvalue with last ten:")
               rdv = dv[0]/dv
               self.log( rdv[-10:])
               qdump(self.rdv,rdv)
               dv[self.mcut:] = 0 * dv[self.mcut:]
               sol2 = divz(1,dv[:self.mcut])
               sol3 = dot(transpose(du[::,:self.mcut]),f0)
               self.psf1 = dot(sol1[::,:self.mcut], (sol2*sol3))
               fitted = add.reduce(self.psf1[::,NewAxis]*basis,0)
               resid = fitted - f0
               vc = dv[0]/dv[self.mcut-1]
               umean = mean(resid)
               urms = sqrt(mean(power(resid,2)))
               uchi2 = sum(power(resid*n0,2))
               uchi2u = uchi2 / (len(resid)-self.mcut)
               self.log( "(CUT,MEAN,RMS,CHI2,RCHI2) = (%9.1f,%12.6f,%12.6f,%12.6f,%12.6f)" % (vc,umean,urms,uchi2,uchi2u))
           else:
               self.log( "Ratio of first eigenvalue with last ten:")
               rdv = 1.0/dv
               self.log( rdv[-10:])
               qdump(self.rdv,rdv)
               cdv = zeros(rdv.shape,Float64)
               if self.vcut == 0: ucv = [0,1]
               else: ucv = [1]
               for cv in ucv:
                  if cv:
                      if self.vcut == 0:
                         qdump("cdv.fits",rdv - cdv)
                         tck = splrep(arange(len(dv)),rdv-cdv,k=1,s=0)
                         vloc = sproot(tck,mest=len(dv))
                         vloc = int(vloc[0])
                         self.vcut = 10*rdv[vloc]
                      rdv = [self.vcut]
                      self.log( "Cutting at %1f." % (self.vcut))
                  for jvc in range(len(rdv)):
                      vc = rdv[jvc]
                      udv = 1.0*greater_equal(dv,max(dv)/vc)
                      jcut = int(sum(udv))
                      if cv: self.log( "Using %d out of %d." % (sum(udv),len(udv)))
                      sol2 = divz(1,dv[:jcut])
                      sol3 = dot(transpose(du[::,:jcut]),f0)
                      self.psf1 = dot(sol1[::,:jcut], (sol2*sol3))
                      fitted = add.reduce(self.psf1[::,NewAxis]*basis,0)
                      resid = fitted - f0
                      umean = mean(resid)
                      urms = sqrt(mean(power(resid,2)))
                      uchi2 = sum(power(divz(resid,n0),2))
                      uchi2u = uchi2 / (len(resid)-self.mcut)
                      if not cv: cdv[jvc] = urms
                      self.log( "(CUT,MEAN,RMS,CHI2,RCHI2) = (%9.1f,%12.6f,%12.6f,%12.6f,%12.6f)" % (vc,umean,urms,uchi2,uchi2u))
           del sol1,sol2,sol3,du,dv,dw,basis
           if self.skyoff: 
              self.log( "Sky= %f" % (self.psf1[ll]))
           # Save the flux ratio for later if reversing.
           self.fluxrat = sum(self.psf1[:ll])
           qdump("psf1.fits",self.psf1)
           self.log( "Flux ratio=%f" %(self.fluxrat))
           if preserve:
              self.log( "Preserving flux?")
              self.psf1[:ll] = self.psf1[:ll] / self.fluxrat
              fluxrat = sum(self.psf1[:ll])
              self.log( "Flux ratio=%f"%fluxrat)
           if self.verb > 1:
             self.psf2 = zeros((pful,pful),Float64)
             pxs = []
             pys = []
             prs = []
             pfs = []
             for k in range(ll):
                 py = self.jj[k]+self.pwid
                 px = self.ii[k]+self.pwid
                 pr = sqrt(self.jj[k]**2 + self.ii[k]**2)
                 self.psf2[py,px] = self.psf1[k]
                 prs.append(pr)
                 pxs.append(self.ii[k])
                 pys.append(self.jj[k])
                 pfs.append( self.psf2[py,px])
             prsf = transpose([pxs,pys,prs,pfs])
             psffile = open("psf.dat","w")
             [psffile.write("%f %f %f %f\n" % tuple(p)) for p in prsf]
             psffile.close()
             for p in self.psf2:
                fmt = pful*"  %7.4f"
                self.log( fmt % tuple(p))
           symm = False
           if symm:
              self.psf2 = (self.psf2+ self.psf2[::-1,::]+ self.psf2[::,::-1]+\
                    self.psf2[::-1,::-1])/4.0
              for k in range(ll):
                  py = self.jj[k]+self.pwid
                  px = self.ii[k]+self.pwid
                  self.psf1[k] = self.psf2[py,px]
           self.match = zeros(self.data.shape,Float64)
           self.grid = zeros(self.data.shape,Float64)
           if basis_sigma is not None:
              # the noise in the convolved image:
              self.csigma = zeros(self.data.shape, Float64)
           dgridx, dgridy = int(self.naxis1/32), int(self.naxis2/32)
           gridin = logical_not(fmod(self.ox+dgridx,dgridx*2.0)) * \
                    logical_not(fmod(self.oy+dgridy,dgridy*2.0))
           gridin = gridin.astype(int8)
           sys.stdout.write("Constructing matched image.\n")
           it0 = time.time()
           if quick_convolve:
              self.match = self.quick_convolve(basisimage, grid=0)
              self.grid = self.quick_convolve(gridin, grid=1)
              if basis_sigma is not None:
                 self.csigma = self.quick_convolve(basis_sigma, grid=0)
           else:
              for k in range(ll):
                 if k % step == 0:
                    sys.stdout.write(".");sys.stdout.flush()
                 self.match += self.component(k,matching=basisimage)
                 self.grid += self.component(k,matching=gridin,grid=1)
                 if basis_sigma is not None:
                    self.csigma += self.component(k, matching=basis_sigma)
           it1 = time.time()
           self.log( "done.\nConstructed matched image in %.4fs." % (it1-it0))
           if basis_sigma is not None:
              self.csigma = sqrt(self.csigma)


    def quick_convolve(self, matching, grid=0):
       '''Use FFT to do a quick convolve if the psf is not spatially varying.'''

       shape = matching.shape
       kshape = (2*self.pwid+1,2*self.pwid+1)
       ksmall = zeros(kshape, Float32)
       ii = self.ii + self.pwid
       jj = self.jj + self.pwid
       for k in range(len(self.ii)):  ksmall[jj[k],ii[k]] = self.psf1[k]

       ksmall = ksmall[::-1,::-1]

       kernel = zeros((kshape[0]+shape[0], kshape[1]+shape[1]), Float32)
       kernel[:kshape[0], :kshape[1]] = ksmall[:,:]
       data = kernel*0.0
       dy = (data.shape[0] - matching.shape[0])//2
       dx = (data.shape[1] - matching.shape[1])//2
       my = matching.shape[0] + dy
       mx = matching.shape[1] + dx
       data[dy:my, dx:mx] = matching[:,:]

       Fdata = fft.rfft2(data)
       sys.stdout.write('.'*8)
       sys.stdout.flush()
       del data
       Fkernel = fft.rfft2(kernel)
       sys.stdout.write('.'*8)
       sys.stdout.flush()
       multiply(Fdata, Fkernel, Fdata)
       del Fkernel
       convolved = fft.irfft2(Fdata, s=kernel.shape)
       sys.stdout.write('.'*8)
       sys.stdout.flush()
       convolved = convolved[kshape[0]-1:shape[0]+kshape[0]-1,kshape[1]-1:shape[1]+kshape[1]-1]
       return(convolved) 

    def component(self,k,matching,grid=0):
        ll = len(self.ii)
        if self.skyoff: s = 1
        else: s = 0
        p, i, j = self.psf1[k], self.ii[k%ll], self.jj[k%ll]
        if s+ll <= k < s+2*ll: p = p*self.oxl
        elif s+2*ll <= k < s+3*ll: p = p*self.oyl
        if k == ll and self.skyoff: return [p,0.0][grid]
        else: return p*QuickShift(matching,i,j)


    def GoCatGo(self,master,verb=0,rev=0,xwin=None,ywin=None, skyoff=0,pwid=0,
                perr=5.0,nmax=40,nord=0,spatial=0,mcut=0,vcut=1e-2,match=1,subt=0,
                registered=0,preserve=0, min_sep=None,
                quick_convolve=0, do_sex=0,
                thresh=3., sexdir="./sex", angles=[0.0], use_db=0, interactive=0,
                starlist=None, diff_size=None, bs=False, crowd=False, usewcs=False):
        self.master = master
        self.skyoff=skyoff
        self.pwid=pwid
        #self.xwin=xwin
        #self.ywin=ywin
        if xwin and ywin:
           self.mask.add_mask(xwin[0], ywin[0], xwin[1], ywin[1], sense='outside')
           self.master.mask.add_mask(xwin[0], ywin[0], xwin[1], ywin[1], sense='outside')
        elif xwin:
           self.mask.add_mask(xwin[0], None, xwin[1], None, sense='outside')
           self.master.mask.add_mask(xwin[0], None, xwin[1], None, sense='outside')
        elif ywin:
           self.mask.add_mask(None, ywin[0], None, ywin[1], sense='outside')
           self.master.mask.add_mask(None, ywin[0], None, ywin[1], sense='outside')
        self.spatial=spatial
        self.rev=rev
        self.vcut=vcut
        self.mcut=mcut
        self.verb=verb
        self.master.verb = verb
        #self.user_masks=user_masks
        self.do_sex=do_sex
        self.sexdir = sexdir
        self.thresh = thresh
        self.master.do_sex=do_sex
        self.master.sexdir = sexdir
        self.master.thresh = thresh
        self.master.crowd = crowd
        self.imread(bs=bs, usewcs=usewcs)
        self.readcat()
        self.master.readcat()
        if self.snpos is not None:
           snx,sny = self.snpos.topixel()
        else:
           snx,sny = None,None
        self.crowd = crowd
        #self.mask.dump_mask(self.image.replace('.fits','_imask.fits'))

        if starlist is not None:
           self.starlist = loadtxt(starlist)
        else:
           self.starlist = None
        if not registered:
           self.compute(nmax=nmax, min_sep=min_sep)
           self.master.compute(nmax=nmax, min_sep=min_sep)
           res = self.objmatch(nord=nord,perr=perr,angles=angles,
                 interactive=interactive, use_db=use_db)
           if res < 0:
              self.log('Failed object match... giving up.')
              return -1
        self.mktemplate(registered, usewcs=False)
        self.mkweight()
        qdump(self.rectemp,self.timage*(self.exptime/self.master.exptime)- \
              self.bg,self.master.image)
        qdump("tempmask.fits", self.timage*greater(self.wt, 0), self.image)
        if self.tsigma is not None:
           qdump(self.recsigma,self.tsigma*(self.exptime/self.master.exptime),
              self.master.image)
        qdump(self.masked,greater(self.wt,0)*(self.data-self.bg),self.image)
        #if subt: qdump(self.difference,self.data-self.timage* \
        #      (self.exptime/self.master.exptime),self.image)

        if match:
           self.mkmatch(preserve,quick_convolve=quick_convolve)
           if self.rev:
              qdump(self.template,(self.match-self.bg),self.image)
           else:
              qdump(self.template,(self.match-self.bg),self.master.image)
           if self.pwid > -1: qdump(self.psfgrid,self.grid)
        if subt:
           ids = indices(self.match.shape)
           if self.rev: 
              if diff_size is not None and snx is not None and sny is not None:
                 dists = sqrt(power(ids[1]-snx,2) + power(ids[0]-sny,2))
                 diff = self.match - where(less(dists,diff_size/self.scale),
                       self.image/self.fluxrat, 0)
              else:
                 diff = (self.match-self.timage/self.fluxrat)
              qdump(self.difference,diff,self.image)
              if snx is not None and sny is not None:
                 qdump(self.diffpostage, (self.match-self.timage/self.fluxrat)\
                       [int(sny)-200:int(sny)+201,int(snx)-200:int(snx)+201],
                       self.image)
                 qdump(self.SNpostage, (self.match)\
                       [int(sny)-200:int(sny)+201,int(snx)-200:int(snx)+201],
                       self.image)
              qdump(self.SN, (self.match - self.bg), self.image)
              if self.sigma is not None:
                 qdump(self.SN.replace('.fits','_sigma.fits'), self.sigma, 
                       self.image)
              qdump(self.template, (self.timage - self.bg), self.master.image)
              if self.sigimage is not None and self.master.sigimage is not None:
                 # Make the noise map of the difference image
                 var = power(self.csigma, 2) + power(self.master.sigma, 2)
                 qdump(self.difference.replace('.fits','_sigma.fits'), sqrt(var), 
                       self.image)
           else: 
              if diff_size is not None and snx is not None and sny is not None:
                 dists = sqrt(power(ids[1]-snx,2) + power(ids[0]-sny,2))
                 diff = self.data - where(less(dists,diff_size/self.scale),
                       self.match, 0)
              else:
                 diff = (self.data-self.match)
              qdump(self.difference,diff,self.image)
              if snx is not None and sny is not None:
                 qdump(self.diffpostage, (self.data-self.match)\
                       [int(sny)-200:int(sny)+201,int(snx)-200:int(snx)+201],
                       self.image)
                 qdump(self.SNpostage, (self.data)\
                       [int(sny)-200:int(sny)+201,int(snx)-200:int(snx)+201],
                       self.image)
              qdump(self.SN,self.data,self.image)
              qdump(self.template,(self.match-self.bg),self.master.image)
              if self.sigimage is not None and self.master.sigimage is not None:
                 # Make the noise map of the difference image
                 var = power(self.csigma, 2) + power(self.sigma, 2)
                 qdump(self.difference.replace('.fits','_sigma.fits'), sqrt(var), 
                       self.image)
        return 0

